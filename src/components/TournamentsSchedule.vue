<template>
  ...
  <v-container>
    <v-row :key="start" v-for="{ name, location, start, end, document, next } in tournaments" class="bg-indigo-lighten-5 ma-auto">
      ...
      <v-col cols="7"><span>{{ name }}</span><br/><span>{{ location }}</span><br/><span v-if="document"><a :href="document">Information</a></span></v-col>
      <v-col cols="4"><span>{{ start }}</span><br/><span>{{ end }}</span></v-col>
    </v-row>
  </v-container>
</template>
